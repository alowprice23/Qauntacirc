...
def test_kolmogorov_complexity_approximation():
    ...
    s_simple = "a" * 1000
    k_simple = kolmogorov_complexity_approximation(s_simple)
    ...
    k_random = kolmogorov_complexity_approximation(s_random)
    assert k_random > 700 # Should be close to the original size

    # Check bytes vs string
    assert k_simple == kolmogorov_complexity_approximation(s_simple.encode('utf-8'))
...
def test_normalized_compression_distance():
    ...
    assert normalized_compression_distance(s1, s3) > 0.8
